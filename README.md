...
### Deliverables
- [Database Design Document (PDF)] `docs/`
...
- **Challenge: Large Dataset Handling.** Sending 1,600+ records caused "Socket Hang Up" errors in Postman.
  - **Solution:** Implemented explicit `Content-Length` headers in API responses.
- **Challenge: Data Synchronization.** Keeping Dictionary and List in sync during POST/DELETE operations.
  - **Solution:** Created unified update logic updating both `transactions` list and `transactions_dict` simultaneously.
- **Challenge: Parse Errors.** Encountered "Invalid character" errors in Postman.
  - **Solution:** Enforced `utf-8` encoding on all `wfile.write` calls.
